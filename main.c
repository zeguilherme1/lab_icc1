...
void addUser(vector* array, int id, char* nome, int idade, float saldoAtual) {
  ...
  strncpy(array->DB[array->size].nome, nome,
          sizeof(array->DB[array->size].nome) - 1);
...
void printVector(vector* array) {
  ...
  for (int i = 0; i < array->size; i++) {
    printf("ID: %d, Nome: %s, Idade: %d, Saldo Atual: %.2f\n", array->DB[i].id,
           array->DB[i].nome, array->DB[i].idade, array->DB[i].saldoAtual);
  }
}

void cleanVector(vector* array) { free(array->DB); }
